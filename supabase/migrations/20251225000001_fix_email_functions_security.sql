-- Fix email functions to ensure they have SECURITY DEFINER
-- This ensures they can insert into email_logs when called from handle_new_user trigger

-- 1. Ensure send_admin_alert_email has SECURITY DEFINER
...
-- 2. Ensure send_welcome_email_to_user has SECURITY DEFINER
...
'Enhanced version that extracts individual fields from details JSONB. Uses SECURITY DEFINER to ensure INSERT permissions.';

COMMENT ON FUNCTION send_welcome_email_to_user(TEXT, UUID, TEXT) IS 
'Sends welcome email to new users. Uses SECURITY DEFINER to ensure INSERT permissions.';
