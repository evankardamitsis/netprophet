-- Remove auto-claim functionality from handle_new_user
-- The find_matching_players function doesn't exist, causing errors
-- Users should claim profiles manually via the notification flow
...
BEGIN
    ...
    BEGIN
        PERFORM public.send_admin_alert_email(
            ...
            ('A new user has registered on NetProphet: ' || NEW.email)::TEXT,
            jsonb_build_object(
    ...
    BEGIN
        PERFORM public.send_welcome_email_to_user(
            NEW.email::TEXT,
            NEW.id,
            COALESCE(NULLIF(trim(user_first_name || ' ' || user_last_name), ''), 'New User')::TEXT
        );
...
'Creates profile, sends admin email (email_logs), creates in-app notification (public.create_admin_notification), welcome email, and queues user for MailerLite subscription. Auto-claim removed.';
