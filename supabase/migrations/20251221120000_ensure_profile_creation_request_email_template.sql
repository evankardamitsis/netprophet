-- Ensure profile_creation_request email template is up to date with all form fields
-- This migration updates the template to include date_of_birth, playing_hand, and age

-- Update the template to include all form fields
UPDATE email_templates
SET 
    subject = '🆕 New Player Profile Creation Request',
    html_content = '
<!DOCTYPE html>
<html>
<head>
...
BEGIN
    ...
    WHERE type = 'profile_creation_request'
        AND language = 'en'
        AND html_content LIKE '%{{date_of_birth}}%'
        AND html_content LIKE '%{{playing_hand}}%'
        AND html_content LIKE '%{{age}}%';

    IF template_count = 0 THEN
        RAISE WARNING 'Profile creation request email template may not have been updated correctly';
    ELSE
        RAISE LOG 'Profile creation request email template updated successfully';
END
IF;
END $$;

COMMENT ON FUNCTION send_profile_creation_request_admin_email
(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) IS 
'Sends admin alert emails for athlete profile creation requests using profile_creation_request template with all form data (date_of_birth, playing_hand, age)';
