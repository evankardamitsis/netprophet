-- Update trigger functions to handle doubles matches
-- This migration updates the existing trigger functions to detect doubles matches
-- and update partnerships and doubles H2H records accordingly

-- Step 1: Update trigger function for match result inserts to handle doubles
...
-- Step 2: Update trigger function for match result updates to handle doubles
...
-- Step 3: Update trigger function for match result deletions to handle doubles
...
-- Step 4: Create function to reverse partnership record (for updates/deletes)
CREATE OR REPLACE FUNCTION public.reverse_partnership_record(
    p_player_1_id UUID,
    p_player_2_id UUID,
...
BEGIN
    ...
    IF v_existing_record IS NULL THEN
        -- No record to reverse
        RETURN;
    END IF;
    
    -- Determine if this partnership won (to reverse)
    IF (p_previous_winner_id = v_player_a_id OR p_previous_winner_id = v_player_b_id) THEN
        v_wins := -1;
        v_losses := 0;
    ELSE
        v_wins := 0;
    ...
    -- Update record (subtract the match)
    IF v_existing_record.total_matches > 1 THEN
        ...
        WHERE player_a_id = v_player_a_id AND player_b_id = v_player_b_id;
    ELSE
        -- Delete record if this was the only match
...
-- Step 5: Create function to reverse doubles H2H record (for updates/deletes)
CREATE OR REPLACE FUNCTION public.reverse_doubles_h2h_record(
    p_team_a_p1_id UUID,
    p_team_a_p2_id UUID,
    p_team_b_p1_id UUID,
    p_team_b_p2_id UUID,
    p_previous_winner_team TEXT
)
RETURNS VOID AS $$
DECLARE
    v_team_a_p1_id UUID;
...
BEGIN
    ...
    -- Determine which team won (to reverse)
    IF p_previous_winner_team = 'team_a' THEN
        v_team_a_wins := -1;
        v_team_b_wins := 0;
    ELSE
        v_team_a_wins := 0;
    ...
    ELSE
        -- Delete record if this was the only match
        DELETE FROM public.doubles_head_to_head
        WHERE team_a_player1_id = v_team_a_p1_id 
          AND team_a_player2_id = v_team_a_p2_id
          AND team_b_player1_id = v_team_b_p1_id
          AND team_b_player2_id = v_team_b_p2_id;
