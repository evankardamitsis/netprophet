-- Force fix the send_admin_alert_email function to use correct status
-- Drop and recreate to ensure the fix takes effect

DROP FUNCTION IF EXISTS send_admin_alert_email
(TEXT, TEXT, JSONB);
...
VALUES
    (
        ...
        template_variables,
        'sent'  -- Use 'sent' instead of 'pending'
        );
END LOOP;
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON FUNCTION send_admin_alert_email IS 'Sends admin alert email to all administrators - fixed status constraint';
