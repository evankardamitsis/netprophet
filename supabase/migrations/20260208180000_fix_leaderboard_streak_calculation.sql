-- Fix streak calculation in update_user_leaderboard_stats
-- Current streak: consecutive wins from most recent bet going backwards until a loss
-- Best streak: longest consecutive winning streak ever (not total wins)
...
BEGIN
    ...
    SELECT COALESCE(SUM(
        CASE WHEN status = 'won' THEN 10 ELSE 0 END +
        CASE WHEN status = 'won' AND multiplier >= 2.0 THEN FLOOR(multiplier * 5) ELSE 0 END +
        CASE
            WHEN status = 'won' AND is_parlay = true AND parlay_id IS NOT NULL THEN
                COALESCE((SELECT FLOOR(final_odds * 10) FROM parlays WHERE id = parlay_id AND final_odds > 1.0), 0)
            ELSE 0
        END
    ), 0)
    INTO new_points
    ...
    WHERE user_id = user_id_param
        ...
        AND resolved_at > COALESCE(
            (SELECT last_leaderboard_update FROM profiles WHERE id = user_id_param),
            '1970-01-01'::timestamp
        );

    -- Apply streak multiplier power-up effect only to new points if active
    IF has_streak_multiplier THEN
        new_points := FLOOR(new_points * 1.5);
    END IF;

    -- Calculate total points (current + new)
    new_leaderboard_points := current_points + new_points;

    -- Calculate current streak: consecutive wins from most recent bet going backwards until a loss
    ...
    LOOP
        ...
        ELSE
            EXIT;  -- Hit a loss, stop counting
        END IF;
    END LOOP;

    -- Calculate best streak: longest consecutive winning streak ever
    max_streak := 0;
    current_streak := 0;
    FOR bet_record IN
        SELECT status
    ...
    LOOP
        ...
        ELSE
            current_streak := 0;  -- Reset on loss
        END IF;
    END LOOP;
    new_best_streak := max_streak;
    ...
    SET
        ...
        best_winning_streak = GREATEST(COALESCE(best_winning_streak, 0), new_best_streak),
...
COMMENT ON FUNCTION update_user_leaderboard_stats(UUID) IS 'Update user leaderboard statistics with correct consecutive streak calculation and power-up effects';

-- Recalculate leaderboard stats for all users who have resolved bets (fixes existing incorrect streak data)
DO $$
DECLARE
    user_record RECORD;
BEGIN
    ...
    LOOP
        PERFORM update_user_leaderboard_stats(user_record.user_id);
    END LOOP;
END $$;
