...
function calculateOdds(
  ...
  let player1Score = 0.5; // Start at 50%
    ...
    player1Score += factors.formAdvantage * 0.10;      // Reduced from 0.12 to 0.10
    player1Score += factors.surfaceAdvantage * 0.08;   // Reduced from 0.10 to 0.08
    player1Score += factors.headToHeadAdvantage * 0.20; // Increased from 0.12 to 0.20
  ...
  const recommendations = generateRecommendations(player1, player2, factors, context);
...
function calculateFactors(player1: PlayerOddsData, player2: PlayerOddsData, context: MatchContext, h2hRecord?: H2HRecordInput) {
  return {
    ntrpAdvantage: calculateNTRPAdvantage(player1, player2),
    formAdvantage: calculateFormAdvantage(player1, player2),
    surfaceAdvantage: calculateSurfaceAdvantage(player1, player2, context.surface),
    ...
    headToHeadAdvantage: calculateHeadToHeadAdvantage(player1, player2, h2hRecord),
    fatigueAdvantage: calculateFatigueAdvantage(player1, player2),
  };
}

function calculateNTRPAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
...
function calculateFormAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
  ...
  const p1WinRate = p1Matches >= 10 ? p1Wins / p1Matches : (p1Wins + 5) / (p1Matches + 10);
  const p2WinRate = p2Matches >= 10 ? p2Wins / p2Matches : (p2Wins + 5) / (p2Matches + 10);
  
  // Calculate recent form with adaptive weighting
  const recentWeights = [0.40, 0.25, 0.20, 0.10, 0.05];
  const p1Last5 = player1.last5;
  const p2Last5 = player2.last5;
  
  const p1RecentForm = p1Last5.reduce((sum, result, index) => 
    sum + (result === 'W' ? (recentWeights[index] || 0) : 0), 0);
  const p2RecentForm = p2Last5.reduce((sum, result, index) => 
    sum + (result === 'W' ? (recentWeights[index] || 0) : 0), 0);
  
  // Adaptive weighting based on match count
  const recentWeight = Math.min(0.8, Math.max(0.5, (p1Matches + p2Matches) / 40));
...
function calculateSurfaceAdvantage(player1: PlayerOddsData, player2: PlayerOddsData, surface: string): number {
  const getSurfaceWinRate = (player: PlayerOddsData, surface: string): number => {
...
function calculateHeadToHeadAdvantage(player1: PlayerOddsData, player2: PlayerOddsData, h2hRecord?: H2HRecordInput): number {
  if (!h2hRecord) {
    return 0;
  }
  ...
  if (h2hRecord.lastMatchResult && h2hRecord.lastMatchDate) {
    ...
    const daysSince = (Date.now() - lastMatchDate.getTime()) / (1000 * 60 * 60 * 24);
    if (daysSince < 180) {
...
function calculateFatigueAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
...
function calculateExperienceAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
...
function calculateMomentumAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
  ...
  const p1Momentum = p1StreakType === 'W' 
    ? Math.sqrt(p1CurrentStreak) 
    : -Math.sqrt(p1CurrentStreak);
  const p2Momentum = p2StreakType === 'W' 
...
function calculateConfidence(player1: PlayerOddsData, player2: PlayerOddsData, factors: any): number {
  let confidence = 0.6;
  ...
  if (p1Matches >= 30 && p2Matches >= 30) confidence += 0.10;
  
  if (player1.surfaceWinRates && player2.surfaceWinRates) confidence += 0.05;
  
  if (player1.headToHeadRecord && player2.headToHeadRecord) confidence += 0.10;
  
  const factorVariance = Math.abs(factors.ntrpAdvantage) + 
                        Math.abs(factors.formAdvantage) + 
                        Math.abs(factors.surfaceAdvantage) +
                        Math.abs(factors.headToHeadAdvantage);
  
  if (factorVariance > 0.6) confidence += 0.10;
  if (factorVariance > 1.0) confidence += 0.05;
  
  return Math.max(0.3, Math.min(0.95, confidence));
}

function generateRecommendations(player1: PlayerOddsData, player2: PlayerOddsData, factors: any, context: MatchContext): string[] {
  ...
  if (Math.abs(factors.headToHeadAdvantage) > 0.1) {
    ...
    if (h2h && h2h.opponentId === opponent.id) {
      ...
      recommendations.push(`${advantaged.firstName} leads H2H vs ${opponent.firstName} ${h2h.wins}-${h2h.losses} (${winRate}% win rate)`);
    }
  }
  ...
  if (Math.abs(factors.ntrpAdvantage) > 0.15) {
    ...
    recommendations.push(`${stronger.firstName} has NTRP advantage (${strongerRating})`);
  }
  ...
  if (Math.abs(factors.surfaceAdvantage) > 0.1) {
    ...
    if (advantaged.surfaceWinRates) {
      const surfaceKey = context.surface.toLowerCase().replace(' ', '') as keyof typeof advantaged.surfaceWinRates;
      const winRate = advantaged.surfaceWinRates[surfaceKey];
      if (winRate) {
        recommendations.push(`${advantaged.firstName} excels on ${context.surface} (${(winRate * 100).toFixed(0)}% win rate)`);
      }
    }
  }
  ...
  if (Math.abs(factors.formAdvantage) > 0.1) {
    ...
    recommendations.push(`${inForm.firstName} in good form (${recentWins}/5 recent wins)`);
  }
    ...
    for (const matchId of matchIds) {
      try {
        ...
        const context: MatchContext = {
          surface: match.tournaments.surface as 'Hard Court' | 'Clay Court' | 'Grass Court' | 'Indoor',
        }

        // Calculate odds using embedded algorithm
        const oddsResult = calculateOdds(playerA, playerB, context)
