...
export async function fetchUserPowerUps(userId: string): Promise<UserPowerUp[]> {
    const { data, error } = await supabase
        .from('user_power_ups')
        .select(`
            *,
...
export async function purchasePowerUp(
    ...
    try {
        ...
        const existingPowerUp = existingPowerUps && existingPowerUps.length > 0 ? existingPowerUps[0] : null;
        ...
        if (existingPowerUp) {
            ...
            if (expiresAt && !existingPowerUp.expires_at) {
                ...
                console.log('Setting expiration date for existing power-up:', expiresAt);
            } else if (expiresAt && existingPowerUp.expires_at) {
                console.log('Keeping existing expiration date:', existingPowerUp.expires_at);
            } else if (expiresAt) {
                // Force set expiration date for time-based power-ups that should have one
                updateData.expires_at = expiresAt;
                console.log('Forcing expiration date for time-based power-up:', expiresAt);
            } else {
                console.log('No expiration date to set. expiresAt:', expiresAt, 'existingPowerUp.expires_at:', existingPowerUp.expires_at);
            }
...
export async function hasActiveStreakMultiplier(userId: string): Promise<boolean> {
    try {
        const { data: userPowerUp, error } = await supabase
            .from('user_power_ups')
            .select('*')
            .eq('user_id', userId)
...
export async function getActivePowerUps(userId: string): Promise<UserPowerUp[]> {
    try {
