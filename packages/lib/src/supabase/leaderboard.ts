...
export class LeaderboardService {
  ...
  private static async calculatePoints(
    ...
    for (const bet of wonBets) {
      ...
      if (bet.is_parlay && bet.parlay_id) {
        // Get parlay details from the parlays table
        ...
        if (parlay && parlay.final_odds > 1.0) {
          totalPoints += Math.floor(parlay.final_odds * 10);
        }
      }
    }
  ...
  private static calculateCurrentStreak(resolvedBets: any[]): number {
    ...
    const sortedBets = resolvedBets.sort(
      (a, b) =>
  ...
  static async getUserStats(userId?: string): Promise<UserStats | null> {
    ...
    const totalWinnings =
      wonBets?.reduce((sum, bet) => sum + (bet.winnings_paid || 0), 0) || 0;
    const totalLosses =
      lostBets?.reduce((sum, bet) => sum + bet.bet_amount, 0) || 0;
    const parlayWins =
      parlayBets?.filter((bet) => bet.status === "won").length || 0;
  ...
  static async getCurrentUserWeeklyRank(): Promise<number | null> {
    ...
    const { data: weeklyStats } = await supabase.rpc(
      "get_weekly_leaderboard_stats"
    );
   ...
   */
  static async updateUserLeaderboardStats(
    userId: string
  ...
  static async getCurrentUserAllTimeRank(): Promise<number | null> {
    ...
    const userEntry = allTimeLeaderboard.find(
  ...
  static async getLeaderboardSummary(timeFrame: "weekly" | "allTime"): Promise<{
    ...
    const topScore = Math.max(
      ...leaderboardData.map((entry) => entry.totalPoints)
    );
    const bestStreak = Math.max(
      ...leaderboardData.map((entry) => entry.currentStreak)
    );
    const averageAccuracy =
      leaderboardData.reduce(
        (sum, entry) => sum + entry.accuracyPercentage,
        0
      ) / totalParticipants;
