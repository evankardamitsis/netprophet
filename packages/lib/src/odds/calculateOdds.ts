...
export const PlayerOddsSchema = z.object({
    ...
    fatigueLevel: z.number().min(0).max(10).optional(), // 0 = fresh, 10 = exhausted
...
export function calculateOdds(
    ...
    player1Score += factors.surfaceAdvantage * 0.10;
    ...
    player1Score += factors.fatigueAdvantage * 0.02;
...
function calculateFactors(player1: PlayerOddsData, player2: PlayerOddsData, context: MatchContext, h2hRecord?: H2HRecordInput): Factors {
    ...
    const surfaceAdvantage = calculateSurfaceAdvantage(player1, player2, context.surface);
    ...
    const headToHeadAdvantage = calculateHeadToHeadAdvantage(player1, player2, h2hRecord);
    
    // Fatigue Advantage (new)
    const fatigueAdvantage = calculateFatigueAdvantage(player1, player2);
...
function calculateFormAdvantage(player1: PlayerOddsData, player2: PlayerOddsData): number {
    ...
    const recentWeights = [0.40, 0.25, 0.20, 0.10, 0.05]; // More emphasis on most recent
...
function calculateConfidence(player1: PlayerOddsData, player2: PlayerOddsData, factors: Factors): number {
 ...
 */
export function calculateExpectedValue(odds: number, stake: number, probability: number): number {
