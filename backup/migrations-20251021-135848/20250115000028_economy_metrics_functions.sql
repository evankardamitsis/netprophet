-- Economy metrics functions for admin dashboard
...
BEGIN
    ...
    FROM (
        SELECT 
            DATE_TRUNC('month', NOW() - (generate_series(0, months_back-1) || ' months')::INTERVAL) as month_series,
            COUNT(DISTINCT p.id) as total_users,
            COUNT(DISTINCT CASE WHEN EXISTS (
                SELECT 1 FROM transactions t 
                WHERE t.user_id = p.id 
                AND t.amount > 0 
                AND DATE_TRUNC('month', t.created_at) = DATE_TRUNC('month', NOW() - (generate_series(0, months_back-1) || ' months')::INTERVAL)
            ) THEN p.id END) as paying_users
        FROM generate_series(0, months_back-1) as gs
        LEFT JOIN profiles p ON DATE_TRUNC('month', p.created_at) <= DATE_TRUNC('month', NOW() - (gs || ' months')::INTERVAL)
...
BEGIN
    ...
    FROM (
        ...
        FROM generate_series(0, months_back-1) as gs
        LEFT JOIN transactions t ON DATE_TRUNC('month', t.created_at) = DATE_TRUNC('month', NOW() - (gs || ' months')::INTERVAL)
        GROUP BY month_series
    ) as monthly_stats
    ORDER BY month_series;
END;
