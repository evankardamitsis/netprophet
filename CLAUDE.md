...
:root {
  ...
  --glow-gold:
    0 0 24px rgba(255, 214, 10, 0.3), 0 0 8px rgba(255, 214, 10, 0.15);
  --glow-green:
    0 0 24px rgba(0, 230, 118, 0.25), 0 0 8px rgba(0, 230, 118, 0.12);
  --glow-orange:
    0 0 24px rgba(255, 107, 43, 0.25), 0 0 8px rgba(255, 107, 43, 0.12);
  --glow-violet:
    0 0 24px rgba(139, 92, 246, 0.25), 0 0 8px rgba(139, 92, 246, 0.12);
  --radius-sm: 8px;
  --radius-md: 12px;
  --radius-lg: 16px;
  --radius-xl: 20px;
...
.animate-fade-in-up {
  animation: fade-in-up 250ms var(--ease-smooth) both;
}
.animate-coin-pop {
  animation: coin-pop 500ms var(--ease-bounce);
}
.animate-odds-pulse {
  animation: odds-pulse 5s ease-in-out infinite;
}
.animate-win-flash {
...
.animate-number-tick {
  animation: number-tick 300ms ease;
}

/* Staggered list reveals */
.np-card-list > *:nth-child(1) {
  animation-delay: 0ms;
}
.np-card-list > *:nth-child(2) {
  animation-delay: 50ms;
}
.np-card-list > *:nth-child(3) {
  animation-delay: 100ms;
}
.np-card-list > *:nth-child(4) {
  animation-delay: 150ms;
}
.np-card-list > *:nth-child(5) {
  animation-delay: 200ms;
}
.np-card-list > *:nth-child(6) {
  animation-delay: 250ms;
}
.np-card-list > *:nth-child(7) {
  animation-delay: 300ms;
}
.np-card-list > *:nth-child(8) {
  animation-delay: 350ms;
}

/* Mobile page padding (clears bottom nav + slip peek) */
.np-page-pad {
  padding-bottom: calc(var(--bottom-nav-h) + 72px);
}
...
export function BottomNav({
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 h-16 bg-[#0F1628]/97 backdrop-blur-xl
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 gap-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 text-[#94A3B8] text-[11px]">
          {isLive && (
            <span
              className="px-2 py-0.5 rounded-full bg-[#FF4545]/12 border border-[#FF4545]/30
      ...
      >
        ...
        <div className="flex items-center justify-center w-5 flex-shrink-0">
          <span
            className="text-[9px] font-black text-[#4B5975] tracking-[0.1em]"
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.06] gap-2 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(isUnderdog1 || isUnderdog2) && (
            <span
              className="px-2 py-0.5 rounded-full bg-[#FF6B2B]/10 border border-[#FF6B2B]/30
        ...
        {isActive && (
          <span
            className="px-2 py-0.5 rounded-full bg-[#00E676]/10 border border-[#00E676]/30
...
function OddsBtn({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[10px] font-bold uppercase tracking-[0.04em] text-center leading-tight
      ...
      {isSelected && (
        <span className="absolute top-2 right-2 text-[#00E676] text-xs font-black">
          ✓
        </span>
      )}
  ...
  {matches.map((match) => (
    <MatchCard
      ...
      player1={{
        ...
        ntrp: match.player1.ntrp,
        odds: match.odds.player1,
      }}
      player2={{
        id: match.player2.id,
        name: match.player2.name,
        ntrp: match.player2.ntrp,
    ...
    >
      ...
      <span
        className={[
          ...
          activeTournament === t.id ? "bg-[#FFD60A]/20" : "bg-white/[0.08]",
        ].join(" ")}
      >
        {t.matchCount}
      </span>
...
3. Leaderboard row style:
   ...
   <div
     className={[
       ...
       isMe
         ? "bg-[#00E676]/05 border-[#00E676]/25"
         : rank <= 3
           ? "bg-[#161F35] border-[#FFD60A]/20"
...
2. Fix "Won" / "Lost" status badges:
   ...
   const STATUS = {
     won: {
       label: "✅ Κερδισμένη",
       class: "bg-[#00E676]/10 border-[#00E676]/30 text-[#00E676]",
     },
     lost: {
       label: "❌ Χαμένη",
       class: "bg-[#FF4545]/10 border-[#FF4545]/30 text-[#FF4545]",
     },
     pending: {
       label: "⏳ Σε Αναμονή",
       class: "bg-[#38BDF8]/10 border-[#38BDF8]/25 text-[#38BDF8]",
     },
     active: {
       label: "🔥 Ενεργή",
       class: "bg-[#FF6B2B]/10 border-[#FF6B2B]/30 text-[#FF6B2B]",
     },
   };
   ```
...
4. Add summary strip at top of page:
   ...
   <div className="flex gap-3 overflow-x-auto pb-2 mb-6 scrollbar-none">
     ...
     <StatPill
       icon="🪙"
       label="Κέρδη"
       value={`+${earnings.toLocaleString("el-GR")}`}
       color="gold"
     />
...
1. Add a clear section separator between real-money and virtual-coin purchases:
   ...
   <div className="mb-8">
     <div className="flex items-center gap-3 mb-1">
       ...
       <span
         className="px-2 py-0.5 rounded-full bg-[#38BDF8]/10 border border-[#38BDF8]/25
                        text-[#38BDF8] text-[10px] font-bold"
       >
         € ΑΓΟΡΑ
       </span>
     </div>
     <p className="text-[13px] text-[#94A3B8] mb-4">
       Τα νομίσματα είναι εικονικά και χρησιμοποιούνται αποκλειστικά εντός της
       πλατφόρμας.
     </p>
     {/* coin pack cards */}
   </div>;
...
2. Make real-money buttons show price explicitly:
   ...
   ```tsx
   <button
     className="w-full py-3 rounded-xl bg-[#FFD60A] text-[#080C18] font-black text-sm
...
## Daily Run (branch: feat/daily-run)

Spec: daily-run-port-spec.md (repo root)
Reference implementation: netprophet-app-demo-v7.html (repo root) — the standalone
prototype, working. Read it before writing UI.
Additive only — do not modify existing prediction/coins code on this branch.

Lives in `apps/web/src`: route group `app/(prototype)/`, `lib/daily/`,
`components/daily/`, `hooks/`. Zero imports from the existing app — if something
is needed, copy it into `lib/daily/`. Local state only, under `np_daily_v1`.
`middleware.ts` exempts `/daily` from the locale redirect; that is the only
existing file the branch touches.
