    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 w-full overflow-auto gap-6">
            ...
            {liveMatches.length > 0 && (
                <div>
                    <h2 className="text-lg font-semibold text-accent mb-4 flex items-center">
                        <span className="w-3 h-3 bg-red-500 rounded-full animate-pulse mr-2" />
                        Live Matches ({liveMatches.length})
                    </h2>
                    <div className="grid gap-4">
                        {liveMatches.map((match) => (
                            <Card key={match.id} className="hover:shadow-md transition-shadow border-red-200 cursor-pointer" onClick={() => onSelectMatch(match)}>
            ...
            {upcomingMatches.length > 0 && (
                <div>
                    <h2 className="text-lg font-semibold text-accent mb-4">Upcoming Matches ({upcomingMatches.length})</h2>
                    <div className="grid gap-4">
                        {upcomingMatches.map((match) => (
                            <Card key={match.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => onSelectMatch(match)}>
                                <CardContent className="p-6">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center space-x-2">
