...
export function LiveMatchesGrid({ liveMatches, sidebarOpen = true, slipCollapsed }: LiveMatchesGridProps) {
    ...
    const activeMatches = filteredLiveMatches.filter(m => !m.locked);
    const lockedMatches = filteredLiveMatches.filter(m => m.locked);
    ...
    return (
        <div className="mb-4 mt-4 lg:mt-6">
            ...
            {/* ── Active (live) match cards — grid ── */}
            {activeMatches.length > 0 && (
                <div className={`grid gap-3 ${gridCols} mb-3`}>
                    {activeMatches.map((match) => {
                        const isUnderdog = Math.abs(match.player1.odds - match.player2.odds) > 2.5;
                        ...
                        return (
                            <div
                                ...
                                className={[
                                    'relative overflow-hidden rounded-2xl border transition-all duration-150 flex flex-col p-4 cursor-pointer',
                                    isUnderdog
                                        ? 'bg-[#161F35] border-[#FF6B2B]/30 hover:border-[#FF6B2B]/50 hover:-translate-y-px'
                                        : 'bg-[#161F35] border-white/[0.06] hover:border-white/[0.12] hover:-translate-y-px',
                                ].join(' ')}
                            >
                                {/* Top shine */}
                                ...
                                <div className="flex items-stretch gap-2 flex-1">
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center text-center px-2 py-3 rounded-xl bg-[#1E2A45] border border-white/[0.06] gap-1.5 min-w-0">
                                        ...
                                        <span className={`text-[26px] font-black tabular-nums leading-none ${p1Fav ? 'text-[#38BDF8]' : 'text-[#FF6B2B]'}`}>
            ...
            {/* ── Locked (upcoming) match cards — horizontal scroll ── */}
            {lockedMatches.length > 0 && (
                <div className="flex gap-3 overflow-x-auto [&::-webkit-scrollbar]:hidden pb-2 snap-x snap-mandatory">
                    {lockedMatches.map((match) => (
                        <div
                            key={match.id}
                            className="relative overflow-hidden rounded-2xl border border-white/[0.04] bg-[#161F35] cursor-not-allowed opacity-60 p-3 min-w-[260px] max-w-[280px] snap-start flex-shrink-0 flex flex-col"
                        >
                            {/* Top shine */}
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] font-bold text-[#4B5975] uppercase tracking-wide truncate max-w-[140px]">
                                        {prepForUppercaseDisplay(match.tournament)}
                                    </span>
                                </div>
                                <span className="text-[10px] text-[#4B5975] tabular-nums flex-shrink-0">{match.time}</span>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/[0.06]">
                                <span className="text-sm font-black text-white tabular-nums">
                                    {match.player1.odds.toFixed(2)}<span className="text-[10px] text-[#4B5975] ml-0.5">×</span>
                                </span>
                                <span className="text-sm font-black text-white tabular-nums">
                                    {match.player2.odds.toFixed(2)}<span className="text-[10px] text-[#4B5975] ml-0.5">×</span>
                                </span>
                            </div>
                        </div>
                    ))}
