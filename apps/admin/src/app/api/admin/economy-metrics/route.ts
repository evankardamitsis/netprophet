...
export async function GET(request: NextRequest) {
    try {
        ...
        const [
            ...
            supabase.rpc('get_coin_flow_data', { time_period: timePeriod, days_back: 30 }),
        ...
        // Extract data
        const coinsInjected = coinsInjectedResult.data?.[0] || { total_coins_injected: 0, previous_period_coins: 0, percentage_change: 0 };
        const coinsBurned = coinsBurnedResult.data?.[0] || { total_coins_burned: 0, previous_period_coins: 0, percentage_change: 0 };
        const payingUsers = payingUsersResult.data?.[0] || { paying_users_count: 0, previous_period_count: 0, percentage_change: 0 };
        const averageCoins = averageCoinsResult.data?.[0] || { average_coins_per_user: 0, previous_period_average: 0, percentage_change: 0 };
        const totalUsers = totalUsersResult.data?.[0] || { total_users_count: 0 };
        const activeUsers = activeUsersResult.data?.[0] || { active_users_count: 0, previous_period_count: 0, percentage_change: 0 };
        ...
        const inflowBreakdown = (inflowBreakdownResult.data || []).map((item: any, index: number) => ({
        ...
        const outflowBreakdown = (outflowBreakdownResult.data || []).map((item: any, index: number) => ({
            name: item.transaction_type,
            value: Number(item.total_amount),
            percentage: Number(item.percentage),
            color: getColorForTransactionType(item.transaction_type, index)
        ...
        const topUsers = (topUsersResult.data || []).map((user: any) => {
            ...
            const daysSinceLastSpend = lastSpend 
                ? Math.floor((Date.now() - new Date(lastSpend).getTime()) / (1000 * 60 * 60 * 24))
                : 999;
        ...
        const conversionRateTrend = (conversionRateResult.data || []).map((item: any) => ({
            month: item.month,
        ...
        return NextResponse.json({
            ...
            data: {
                summary: {
                    ...
                    averageCoinBalance: Math.round(Number(averageCoins.average_coins_per_user)),
                    conversionRate: calculateConversionRate(payingUsers.paying_users_count, totalUsers.total_users_count),
                    burnRatio: calculateBurnRatio(coinsInjected.total_coins_injected, coinsBurned.total_coins_burned)
                },
                trends: {
...
function getDefaultColor(index: number): string {
    const colors = ['#10b981', '#f59e0b', '#3b82f6', '#8b5cf6', '#06b6d4', '#ef4444', '#dc2626', '#f97316'];
    return colors[index % colors.length];
}

// Helper function to calculate conversion rate
function calculateConversionRate(payingUsers: number, totalUsers: number): number {
